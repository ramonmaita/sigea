...
<head>
    ...
    <title>ORDEN DE SALIDA DE BIENES N° {{ $ordenSalida->numero_orden_salida }}</title>
    <style>
        @page {
            margin-top: 120px;
...
<body>
    ...
    @php
        $watermarkText = null;
        if ($ordenSalida->estado_orden === 'Borrador') {
            $watermarkText = 'BORRADOR';
        } elseif ($ordenSalida->estado_orden === 'Anulada') {
        ...
        $subtotalItems = 0;
         $totalGeneral = 0;
        foreach ($ordenSalida->biens as $bien) {
    ...
    <main class="content">
        <div class="bold text-center" style="font-size: 12pt; margin-bottom: 5px;">ORDEN DE SALIDA DE BIENES</div>
        {{-- ENCABEZADO DEL DOCUMENTO --}}
        <table border="1" cellspacing="0" ceelpadding="0">
            <tr>
                <td colspan="2" style="width: 31%"><span class="titulos">1.UNIDAD SOLICITANTE:</span><br> {{ $detalleOficina->nombre_oficina }}</td>
                <td><span class="titulos">2.Nº.:</span><br> {{ $ordenSalida->numero_orden_salida }}</td>
            </tr>
            <tr>
                <td  style="width: 60%"><span class="titulos">3.NOMBRE Y APELLIDO:</span><br> {{ $ordenSalida->persona_responsable_retiro }}</td>
                <td><span class="titulos">C.I:</span><br> {{ $ordenSalida->cedula_responsable_retiro }}</td>
                <td><span class="titulos">4. FECHA:</span><br>  {{ $ordenSalida->numero_orden_salida }}</td>
            </tr>
            <tr>
                <td colspan="3" style="height: 50px;" valign="top">
                    <span class="titulos">5. MOTIVO:</span> {{ $ordenSalida->justificacion }}
                </td>
            </tr>
            <tr>
                <th colspan="3">
                    PARA SER LLENADO SOLO EN CASO DE REPARACION
                </th>
            </tr>
            <tr>
                <td colspan="3">
                    <span class="titulos">6. PROVEEDOR: </span>
                    {{ $ordenSalida->proveedor_nombre ?? 'N/A' }}
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <span class="titulos">7. DIRECCION:</span> {{ $ordenSalida->proveedor_direccion ?? 'N/A' }}
                </td>
                <td>
                    <span class="titulos">TELEFONO:</span> {{ $ordenSalida->proveedor_telefono ?? 'N/A' }}
                </td>
            </tr>
        ...
        <table class="item-table" border="1">
            ...
            <tbody>
                @forelse ($ordenSalida->biens as $index => $bien)
                    <tr>
                ...
                @for ($i = count($ordenSalida->biens); $i < 20; $i++)
                    <tr>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
        ...
        <table border="1">
            ...
            <tr>
                ...
                <th align="center" valign="top" style="width:25%; height: 100px;">15. BIENES PUBLICOS</th>
                <th align="center" valign="top" style="width:25%; height: 100px;">16. OFICINA DE GESTION ADMINISTRATIVA</th>
            </tr>
        </table>
